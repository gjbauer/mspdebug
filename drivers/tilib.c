...
static void event_notify(uint32_t msg_id, uint32_t w_param,
			 int32_t l_param, int32_t client_handle_unused)
{
	(void)w_param;
	(void)l_param;
...
{
	int32_t err = tilib_api->MSP430_Error_Number();
...
static int32_t ti_erase_type(device_erase_type_t e)
{
	switch (e) {
...
{
	struct tilib_device *dev = (struct tilib_device *)dev_base;
	int32_t regbuf[DEVICE_NUM_REGS];
	int i;
...
static void fw_progress(unsigned int msg_id, uint32_t w_param,
			uint32_t l_param, int32_t client_handle_unused)
{
	(void)l_param;
	(void)client_handle_unused;
...
static int do_init(struct tilib_device *dev, const struct device_args *args)
{
	...
	if (tilib_api->MSP430_EEM_Init(event_notify, (int32_t)dev,
