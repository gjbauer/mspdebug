...
static STATUS_T new_Initialize(char *port, int32_t *version)
{
	int32_t nv;
	int r;
...
static STATUS_T new_VCC(int32_t voltage)
{
	return napi.MSP430_VCC(voltage);
}

static STATUS_T new_Configure(int32_t mode, int32_t value)
{
...
static STATUS_T new_GetFoundDevice(char *FoundDevice, int32_t count)
{
...
static STATUS_T new_Close(int32_t vccOff)
{
	return napi.MSP430_Close(vccOff);
}

static STATUS_T new_Memory(int32_t address, char *buffer, int32_t count, int32_t rw)
{
	return napi.MSP430_Memory(address, buffer, count, rw);
}

static STATUS_T new_Reset(int32_t method, int32_t execute, int32_t releaseJTAG)
{
	return napi.MSP430_Reset(method, execute, releaseJTAG);
}

static STATUS_T new_Erase(int32_t type, int32_t address, int32_t length)
{
...
static const char *new_Error_String(int32_t errNumber)
{
	return napi.MSP430_Error_String(errNumber);
}

static STATUS_T new_GetNumberOfUsbIfs(int32_t* number)
{
	int32_t nn;
	int r;
...
static STATUS_T new_GetNameOfUsbIf(int32_t idx, char **name, int32_t *status)
{
	int32_t ns;
	int r;
...
static STATUS_T new_Registers(int32_t *registers, int32_t mask, int32_t rw)
{
	int32_t nr[16];
	int i;
...
static STATUS_T new_Run(int32_t mode, int32_t releaseJTAG)
{
	return napi.MSP430_Run(mode, releaseJTAG);
}

static STATUS_T new_State(int32_t *state, int32_t stop, int32_t *pCPUCycles)
{
	int32_t ns;
	int32_t nc;
	int r;
...
static STATUS_T new_EEM_Init(DLL430_EVENTNOTIFY_FUNC callback,
			     int32_t clientHandle, MessageID_t *pMsgIdBuffer)
{
...
struct tilib_old_api {
	/* MSP430.h */
	STATUS_T TIDLL (*MSP430_Initialize)(char *port, int32_t *version);
	STATUS_T TIDLL (*MSP430_VCC)(int32_t voltage);
	STATUS_T TIDLL (*MSP430_Configure)(int32_t mode, int32_t value);
	STATUS_T TIDLL (*MSP430_OpenDevice)(char *Device, char *Password,
					     int32_t PwLength, int32_t DeviceCode,
					     int32_t setId);
	STATUS_T TIDLL (*MSP430_GetFoundDevice)(char *FoundDevice,
						 int32_t count);
	STATUS_T TIDLL (*MSP430_Close)(int32_t vccOff);
	STATUS_T TIDLL (*MSP430_Memory)(int32_t address, char *buffer,
					 int32_t count, int32_t rw);
	STATUS_T TIDLL (*MSP430_Reset)(int32_t method, int32_t execute,
					int32_t releaseJTAG);
	STATUS_T TIDLL (*MSP430_Erase)(int32_t type, int32_t address, int32_t length);
	STATUS_T TIDLL (*MSP430_Secure)(void);
	STATUS_T TIDLL (*MSP430_Error_Number)(void);
	const char *TIDLL (*MSP430_Error_String)(int32_t errNumber);

	STATUS_T TIDLL (*MSP430_GetNumberOfUsbIfs)(int32_t* number);
	STATUS_T TIDLL (*MSP430_GetNameOfUsbIf)(int32_t idx, char **name,
						 int32_t *status);

	/* MSP430_Debug.h */
	STATUS_T TIDLL (*MSP430_Registers)(int32_t *registers, int32_t mask,
					    int32_t rw);
	STATUS_T TIDLL (*MSP430_Run)(int32_t mode, int32_t releaseJTAG);
	STATUS_T TIDLL (*MSP430_State)(int32_t *state, int32_t stop,
					int32_t *pCPUCycles);
...
static STATUS_T old_Initialize(char *port, int32_t *version)
{
    int32_t v;
    STATUS_T ret = old.MSP430_Initialize(port, &v);
    if (ret >= 0)
        *version = (int32_t)v;
    return ret;
}

static STATUS_T old_VCC(int32_t voltage)
{
    return old.MSP430_VCC(voltage);
}

static STATUS_T old_Configure(int32_t mode, int32_t value)
{
...
static STATUS_T old_GetFoundDevice(char *FoundDevice, int32_t count)
{
    return old.MSP430_GetFoundDevice(FoundDevice, count);
}

static STATUS_T old_Close(int32_t vccOff)
{
    return old.MSP430_Close(vccOff);
}

static STATUS_T old_Memory(int32_t address, char *buffer, 
                           int32_t count, int32_t rw)
{
    return old.MSP430_Memory(address, buffer, count, rw);
}

static STATUS_T old_Reset(int32_t method, int32_t execute, 
                          int32_t releaseJTAG)
{
    return old.MSP430_Reset(method, execute, releaseJTAG);
}

static STATUS_T old_Erase(int32_t type, int32_t address, int32_t length)
{
...
static const char *old_Error_String(int32_t errNumber)
{
    return old.MSP430_Error_String(errNumber);
}

static STATUS_T old_GetNumberOfUsbIfs(int32_t* number)
{
    int32_t n;
    STATUS_T ret = old.MSP430_GetNumberOfUsbIfs(&n);
    if (ret >= 0)
        *number = (int32_t)n;
    return ret;
}

static STATUS_T old_GetNameOfUsbIf(int32_t idx, char **name, int32_t *status)
{
    int32_t s;
    STATUS_T ret = old.MSP430_GetNameOfUsbIf(idx, name, &s);
    if (ret >= 0)
        *status = (int32_t)s;
    return ret;
}

static STATUS_T old_Registers(int32_t *registers, int32_t mask, int32_t rw)
{
    return old.MSP430_Registers(registers, mask, rw);
}

static STATUS_T old_Run(int32_t mode, int32_t releaseJTAG)
{
    return old.MSP430_Run(mode, releaseJTAG);
}

static STATUS_T old_State(int32_t *state, int32_t stop, int32_t *pCPUCycles)
{
...
int tilib_api_init(void)
{
	...
	if(res == -1 || res >= sizeof(libpath)) {
		printc_err("MSPDEBUG_TILIB_PATH specifies too int32_t a path\n");
